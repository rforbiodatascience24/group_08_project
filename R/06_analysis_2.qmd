...
estimates |> 
  ...
  geom_point(data = estimates |> filter(p_significance == "yes"), 
                        color = "turquoise", alpha = 0.6) +
  geom_point(data = estimates |> filter(p_significance == "no"), 
                        color = "tomato", alpha = 0.3) +
  geom_text_repel(data = estimates |> filter(p_significance == "yes"), 
  ...
  labs(title = "Genes Associated with Early Metastasis in Small Node-Negative Breast Carcinoma",
       ...
       x = "Estimates",
       y = "- log10( p-value )",
       caption = "Data from DOI: 10.1002/gcc.20820") + ########### <- skal dette slettes?
