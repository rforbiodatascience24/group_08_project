...
estimates |> 
  filter(q.value < 1) |> # filter out insignificant genes
  ggplot(aes(x = estimate, # set x as estimate 
             y = fct_reorder(gene, # set y as gene 
                             estimate))) +
  geom_point() +
  ...
  x = "Estimates (95% Confidence Intervals)",
  y = "Gene",
  caption = "Genes identified as significant (by q-value) after treatment with pembrolizumab, exemestane, and leuprolide."
) +
  theme(plot.title.position = "plot")
```
