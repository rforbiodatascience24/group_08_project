...
density_plot_of_gene_classes <- 
  ...
  facet_wrap("type") +
  labs(title = "Density plot of code class",
       x = "Mean Expression ",
       y = "Density",
       fill = "Gene Class") + 
...
ggsave(here("results/04_density_plot_of_gene_classes.png"), plot = density_plot_of_gene_classes, width = 10, height = 6)

print(density_plot_of_gene_classes)
```
...
top2_fold_change <- describe_dat_aug_03 |> 
  group_by(code_class) |> 
 mutate(
...
ggsave(here("results/04_top2_fold_change.png"), plot = top2_fold_change, width = 10, height = 6)
```
