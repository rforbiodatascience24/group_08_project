...
```{r}
# Transform data to pair pre-count and post-count values by person
wide_data <- long_data %>%
  filter(str_detect(count_type, "pre_count|post_count")) %>%
  mutate(person = str_extract(count_type, "\\d+")) %>%
  pivot_wider(names_from = count_type, values_from = count_value) %>%
  drop_na() # Remove rows with NA values

# Example scatterplot for a single pair of pre/post counts
ggplot(wide_data, aes(x = pre_count_1, y = post_count_1, color = as.factor(person))) +
  geom_point(size = 3) +
  labs(
    title = "Scatter Plot of Pre-count vs Post-count Values",
    x = "Pre-count Values",
    y = "Post-count Values",
    color = "Person"
  ) +
